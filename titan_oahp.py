...
class OAHP:
    ...
    async def generate_action_plan(self, directive: str):
        prompt = f"[DIRECTIVE]: {directive}\n[TASK]: Decompose into 4 technical steps. Return ONLY a JSON list formatted like:[{{'step': 1, 'task': '...'}}]"
        raw = await self.brain.think(prompt, mode="PLANNING", use_tools=False)
        try:
            # Cleaning the output for JSON parsing
            clean = raw.strip().replace("```json", "").replace("```", "")
            self.active_plan = json.loads(clean)
            return self.active_plan
        except Exception as e: 
            print(f"OAHP Plan Error: {e}")
            return None
