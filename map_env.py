...
def map_directory_tree(root_dir, max_depth=2):
    ...
    if not os.path.exists(root_dir):
        ...
        os.system(f"dd if=/dev/urandom of={root_dir}/nexus_core/shards/logic_alpha.bin bs=1M count=12 2>/dev/null")
        os.system(f"dd if=/dev/urandom of={root_dir}/ktrp_telemetry/stream.log bs=1M count=5 2>/dev/null")
        os.system(f"dd if=/dev/urandom of={root_dir}/init_vector.dat bs=1M count=1 2>/dev/null")
